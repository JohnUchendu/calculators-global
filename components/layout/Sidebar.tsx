...
export default function Sidebar() {
  ...
  // Only show categories that have ready tools
  const readyCategories = CATEGORIES.filter((cat) =>
    TOOLS_BY_CATEGORY[cat].some((t) => t.ready !== false)
  );

  const activeCategory = readyCategories.find((cat) =>
    TOOLS_BY_CATEGORY[cat].some((t) => pathname === `/${t.slug}`)
  );
                  ...
                  {tools.map((tool) => {
                    ...
                    return (
                      <li key={tool.slug}>
                        ...
                        >
                          <span className={`w-0.5 h-4 rounded-full transition-all ${isActive ? colors.dot : "bg-transparent"}`} />
                          <span className="text-[9px] font-bold tracking-wider uppercase text-gray-400 w-8 shrink-0">
                            {tool.icon}
                          </span>
                          <span className="truncate">{tool.shortTitle}</span>
                        </Link>
